...
exports.saveBook = async (req, res) => {
  ...
  try {
    ...
    const query = `insert into books_detail (book_title,book_desc,book_img,genre_id,book_publication,book_publication_year,book_ISBN,isdeleted) values (?,?,?,?,?,?,?,?)`;
...
exports.editBook = async (req, res) => {
  try {
    ...
    let query = `select bd.prime_book_id,bd.book_title,bd.book_desc,bd.genre_id,bd.book_img,bd.book_publication,bd.book_publication_year,bd.book_ISBN,group_concat(ba.author_id separator ',')as author_id from books_detail as bd inner join books_author as ba on bd.prime_book_id=ba.prime_book_id 
...
exports.updateBook = async (req, res) => {
  try {
    ...
    if (req.file) {
      query =
        "update books_detail set book_title = ? , book_desc = ? ,book_img=? ,genre_id = ? , book_publication = ? , book_publication_year = ? , book_ISBN = ? where prime_book_id = ? ";
      params = [
        title,
        desc,
        "/uploads/" + req.file.filename,
      ...
      query =
        "update books_detail set book_title = ? , book_desc = ? ,genre_id = ? , book_publication = ? , book_publication_year = ? , book_ISBN = ? where prime_book_id = ? ";
      params = [
        title,
        desc,
        genreid,
...
exports.adminHome = async (req, res) => {
  try {
    ...
    let query4 = `select count(request_id) as totalrequest from requests`;
...
exports.showBooksById = async (req, res) => {
  try {
    ...
    res.render("searchBooksByUser2.ejs", { readerID: readerId, Status: status, searchby: req.query.searchby, searchValue: req.query.searchvalue });
  } catch (error) {
    logger.error(error);
  }
};
